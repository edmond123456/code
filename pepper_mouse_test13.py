...
Created on 2018/05/24
 ...
 test9には3Hzで位置と速度を取ることができた(1/10の時刻でデータを取る)
 test10より速度を倍にする
  velocity in test10
    vecx = -(y[i] - y[i - 1]) / 20.0
    vecy = -(x[i] - x[i - 1]) / 20.0
...
while 1:
    ...
    x[i] = posx
    y[i] = posy
    vmx[i] = (x[i] - x[i - 1]) / (t[i] - t[i - 1])
    vmy[i] = (y[i] - y[i - 1]) / (t[i] - t[i - 1])
    ...
    vx[i] = vecx
    vy[i] = vecy
    motion.moveToward(vx[i], vy[i], 0.0, [["MaxVelXY", MaxVelXY]])
